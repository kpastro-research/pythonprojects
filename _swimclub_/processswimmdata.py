...
def convertcentisecond(centiseconds_param):
    ...
    minutes = int(minutes_seconds) / 60 # assuming centisecond could be more then 60 , here we convert to a minutes
    seconds = int(minutes_seconds) - minutes * 60
    return minutes + ":", str(seconds) + "." + centiseconds # return average
...
def process_swim_data(swimmingData):
    ...
    for value in linevalues:
        ...
        swimmerTimings.append((value, "=>", total_centisecond))
...
def get_data_from_file_name(filename):
    ...
    swimmerData.append("name", "=>", swimmerName)
    swimmerData.append("agegroup", "=>", underAgeGroup)
    swimmerData.append("distance", "=>", swimDistanceInMeters)
    swimmerData.append("stroke", "=>", swimStroke)
    return swimmerData
...
def process_swim_data_file(filename):
    ...
    with  open(filename, 'r') as file:
        for swimmingData in file.readlines():
            swimmerData.swimmerData(process_swim_data(swimmingData))
    return swimmerData
